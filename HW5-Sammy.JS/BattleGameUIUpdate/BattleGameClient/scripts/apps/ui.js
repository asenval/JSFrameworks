...
define(["jquery", "q", "class", "mustache"], function ($, Q, Class, mustache) {
	...
	BattleGameNS.ui = (function () {
		...
		var generateLoginButtons = function (selector) {
		    ...
		    $("<button id='btn-game'>Game</button>").appendTo(container).addClass('button').hide();
		    $("<button id='btn-create'>Create Game</button>").appendTo(container).addClass('button').css({ 'width': '160px' }).hide();//.css({ 'position': 'absolute', 'left': '67%', 'top': '0', 'width': '160px' });
		    $("<button id='btn-scores'>Scores</button>").appendTo(container).addClass('button').hide();//.css({ 'position': 'absolute', 'left': '80%', 'top': '0' });
