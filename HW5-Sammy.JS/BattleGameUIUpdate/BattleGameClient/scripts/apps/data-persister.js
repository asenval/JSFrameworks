...
define(["httpRequester", "class", "cryptojs-sha1"], function (httpRequester, Class, CryptoJS) {

	BattleGameNS.persisters = (function(){
		...
		var UserPersister = Class.create({
			...
			logout: function () {
				var self = this;
				return httpRequester.putJson(this.serviceRootUrl + "logout/" + this._getSessionKey()).
					then(function (data) {
						self._clearUserData();
					});
			},
		...
		var GamePersister = Class.create({
			...
			start: function (gameId) {
				return httpRequester.putJson(this.serviceRootUrl + gameId + "/start/" + this.userPersister._getSessionKey());
			},
	...
	return BattleGameNS.persisters;
});
